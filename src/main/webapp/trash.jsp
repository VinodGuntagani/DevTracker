...
if (activeTab == null || activeTab.isEmpty()) activeTab = "roadmaps";
...
.tab-btn:hover { background: rgba(255,255,255,0.7); color: #1a1a1a; text-decoration: none; }
...
.tab-btn.active .tab-count { background: #EEEDFE; color: #534AB7; }
.tab-count.has-items { background: #fff0f0; color: #a32d2d; }
.tab-btn.active .tab-count.has-items { background: #fff0f0; color: #a32d2d; }
...
.trash-card-left { flex: 1; min-width: 0; }
...
.trash-hierarchy-row i { font-size: 13px; flex-shrink: 0; }
...
/* deleted-at / info pill */
.deleted-pill {
    display: inline-flex; align-items: center; gap: 4px;
    padding: 2px 8px; border-radius: 99px; font-size: 11px;
    background: #fff8f8; border: 0.5px solid #f5c1c1; color: #a32d2d;
}
...
.trash-empty p { font-size: 13px; color: #bbb; margin: 0; }
.trash-empty .empty-hint { font-size: 12px; color: #d0d0d0; }
...
.danger-zone-label {
    ...
    color: #c0766a; margin-bottom: 3px;
}
.danger-zone-text { font-size: 13px; color: #7a2020; }
.danger-zone-text strong { font-weight: 600; }
...
@media (max-width: 640px) {
    .trash-card { flex-direction: column; gap: 0.75rem; }
    .trash-card-actions { flex-direction: row; width: 100%; }
...
<div class="layout">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="nav">
            ...
            <a class="nav-item" href="ai-roadmap.jsp"><i class="ti ti-robot"></i> AI Tools</a>
            <a class="nav-item" href="viewAIRoadmaps"><i class="ti ti-layout-list"></i> AI Roadmaps</a>
            <div class="nav-section-label">Manage</div>
            <a class="nav-item active" href="trash.jsp"><i class="ti ti-trash"></i> Trash</a>
        </nav>
    ...
    <main class="main">
        ...
        <div class="page-header">
            <div>
                <div class="breadcrumb">
                    ...
                    <a href="subjects.jsp?roadmapId=<%=roadmapId%>">Subjects</a>
                    <i class="ti ti-chevron-right"></i>
                    <% } %>
                    <span>Trash</span>
                </div>
                ...
                <div class="page-sub">
                    ...
                    <% } else { %>
                        <%=totalDeleted%> deleted item<%=totalDeleted != 1 ? "s" : ""%> — restore or permanently delete
                    <% } %>
                </div>
            </div>
            <div class="header-actions">
                <% if (hasRoadmapContext) { %>
                <a class="btn" href="subjects.jsp?roadmapId=<%=roadmapId%>"><i class="ti ti-arrow-left"></i> Back to subjects</a>
                <% } else { %>
                <a class="btn" href="dashboard.jsp"><i class="ti ti-arrow-left"></i> Back</a>
                <% } %>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon si-purple"><i class="ti ti-map"></i></div>
                <div>
                    <div class="stat-val"><%=deletedRoadmaps.size()%></div>
                    <div class="stat-label">Roadmaps</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon si-blue"><i class="ti ti-books"></i></div>
                <div>
                    <div class="stat-val"><%=deletedSubjects.size()%></div>
                    <div class="stat-label">Subjects</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon si-teal"><i class="ti ti-notebook"></i></div>
                <div>
                    <div class="stat-val"><%=deletedTopics.size()%></div>
                    <div class="stat-label">Topics</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:#FFF3E0;color:#854F0B;"><i class="ti ti-list-check"></i></div>
                <div>
                    <div class="stat-val"><%=deletedSubTopics.size()%></div>
        ...
        <div class="tab-bar">
            <a class="tab-btn <%="roadmaps".equals(activeTab)?"active":""%>"
               href="?tab=roadmaps<%=hasRoadmapContext?"&roadmapId="+roadmapId:""%>">
                <i class="ti ti-map"></i> Roadmaps
                <span class="tab-count <%=deletedRoadmaps.size()>0?"has-items":""%>"><%=deletedRoadmaps.size()%></span>
            </a>
            <a class="tab-btn <%="subjects".equals(activeTab)?"active":""%>"
               href="?tab=subjects<%=hasRoadmapContext?"&roadmapId="+roadmapId:""%>">
                <i class="ti ti-books"></i> Subjects
                <span class="tab-count <%=deletedSubjects.size()>0?"has-items":""%>"><%=deletedSubjects.size()%></span>
            </a>
            <a class="tab-btn <%="topics".equals(activeTab)?"active":""%>"
               href="?tab=topics<%=hasRoadmapContext?"&roadmapId="+roadmapId:""%>">
                <i class="ti ti-notebook"></i> Topics
                <span class="tab-count <%=deletedTopics.size()>0?"has-items":""%>"><%=deletedTopics.size()%></span>
            </a>
            <a class="tab-btn <%="subtopics".equals(activeTab)?"active":""%>"
               href="?tab=subtopics<%=hasRoadmapContext?"&roadmapId="+roadmapId:""%>">
                <i class="ti ti-list-check"></i> Subtopics
                <span class="tab-count <%=deletedSubTopics.size()>0?"has-items":""%>"><%=deletedSubTopics.size()%></span>
            </a>
        </div>
                ...
                <div class="trash-card">
                    <div class="trash-card-left">
                        ...
                        <div class="trash-card-meta">
                            <% if (r.isAi()) { %>
                            <span class="badge badge-ai"><i class="ti ti-robot" style="font-size:10px"></i> AI</span>
                            <% } %>
                            <% if (r.getStartDate() != null) { %>
                            <span class="deleted-pill"><i class="ti ti-calendar" style="font-size:11px"></i> <%=r.getStartDate()%> – <%=r.getTargetDate()%></span>
                            <% } %>
                    ...
                    <div class="trash-card-actions">
                        <form action="restoreRoadmap" method="post" style="margin:0">
                            ...
                            <button class="btn btn-sm btn-primary" type="submit"><i class="ti ti-restore"></i> Restore</button>
                        </form>
                        <form action="purgeRoadmap" method="post" style="margin:0"
                              onsubmit="return confirm('Permanently delete \'<%=r.getTitle()%>\'? This cannot be undone.')">
                            <input type="hidden" name="id" value="<%=r.getId()%>">
                            <button class="btn btn-sm btn-danger" type="submit"><i class="ti ti-trash-x"></i> Delete</button>
                        </form>
                    </div>
                </div>
                <% } %>
        ...
        <div class="tab-panel <%="subjects".equals(activeTab)?"active":""%>">
            ...
            <div class="trash-list">
                ...
                for (Subject s : deletedSubjects) {
                    ...
                    try { parentRoadmap = roadmapDAO.getRoadmapById(s.getRoadmapId()); } catch (Exception e) {}
                %>
                <div class="trash-card">
                    <div class="trash-card-left">
                        ...
                        <div class="trash-hierarchy">
                            ...
                            <% } %>
                            <div class="trash-hierarchy-row current <%=parentRoadmap!=null?"tree-indent-1":""%>">
                                <% if (parentRoadmap != null) { %><span class="tree-connector">└</span><% } %>
                    ...
                    <div class="trash-card-actions">
                        <form action="restoreSubject" method="post" style="margin:0">
                            ...
                            <input type="hidden" name="roadmapId" value="<%=s.getRoadmapId()%>">
                            <button class="btn btn-sm btn-primary" type="submit"><i class="ti ti-restore"></i> Restore</button>
                        </form>
                        <form action="purgeSubject" method="post" style="margin:0"
                              onsubmit="return confirm('Permanently delete \'<%=s.getName()%>\'? This cannot be undone.')">
                            <input type="hidden" name="id" value="<%=s.getId()%>">
                            <input type="hidden" name="roadmapId" value="<%=s.getRoadmapId()%>">
                            <button class="btn btn-sm btn-danger" type="submit"><i class="ti ti-trash-x"></i> Delete</button>
                        </form>
        ...
        <div class="tab-panel <%="topics".equals(activeTab)?"active":""%>">
            ...
            <div class="trash-empty">
                <div class="empty-icon"><i class="ti ti-notebook-off"></i></div>
            ...
            <div class="trash-list">
                ...
                for (Topic t : deletedTopics) {
                    ...
                    Roadmap parentRoadmap = null;
                    try { parentSubject = subjectDAO.getSubjectById(t.getSubjectId()); } catch (Exception e) {}
                    if (parentSubject != null) {
                        try { parentRoadmap = roadmapDAO.getRoadmapById(parentSubject.getRoadmapId()); } catch (Exception e) {}
                    }
                %>
                <div class="trash-card">
                    <div class="trash-card-left">
                        ...
                        <div class="trash-hierarchy">
                            ...
                            <div class="trash-hierarchy-row current tree-indent-2">
                                <span class="tree-connector">└</span>
                                <i class="ti ti-notebook" style="color:#0d9488"></i>
                                <%=t.getName()%>
                            </div>
                        </div>
                        <div class="trash-card-meta">
                    ...
                    <div class="trash-card-actions">
                        ...
                        <form action="purgeTopic" method="post" style="margin:0"
                              onsubmit="return confirm('Permanently delete \'<%=t.getName()%>\'? This cannot be undone.')">
                            <input type="hidden" name="id" value="<%=t.getId()%>">
                            <input type="hidden" name="roadmapId" value="<%=roadmapId%>">
                            <button class="btn btn-sm btn-danger" type="submit"><i class="ti ti-trash-x"></i> Delete</button>
                        </form>
        ...
        <div class="tab-panel <%="subtopics".equals(activeTab)?"active":""%>">
            ...
            <div class="trash-list">
                ...
                for (SubTopic st : deletedSubTopics) {
                    ...
                    Subject parentSubject = null;
                    Roadmap parentRoadmap = null;
                    try { parentTopic = topicDAO.getTopicById(st.getTopicId()); } catch (Exception e) {}
                    if (parentTopic != null) {
                        try { parentSubject = subjectDAO.getSubjectById(parentTopic.getSubjectId()); } catch (Exception e) {}
                    }
                    if (parentSubject != null) {
                        try { parentRoadmap = roadmapDAO.getRoadmapById(parentSubject.getRoadmapId()); } catch (Exception e) {}
                    }
                    String diff = st.getDifficulty() != null ? st.getDifficulty().toLowerCase().trim() : "medium";
                    String diffClass = diff.equals("easy") ? "difficulty-badge easy"
                                     : diff.equals("hard") ? "difficulty-badge hard"
                ...
                <div class="trash-card">
                    ...
                    <div class="trash-card-actions">
                        ...
                        <form action="purgeSubTopic" method="post" style="margin:0"
                              onsubmit="return confirm('Permanently delete \'<%=st.getName()%>\'? This cannot be undone.')">
                            <input type="hidden" name="id" value="<%=st.getId()%>">
                            <input type="hidden" name="roadmapId" value="<%=roadmapId%>">
                            <button class="btn btn-sm btn-danger" type="submit"><i class="ti ti-trash-x"></i> Delete</button>
                        </form>
        ...
        <div class="danger-zone">
            <div>
                ...
                <div class="danger-zone-text">
                    <strong>Permanently delete everything</strong> — removes all <%=totalDeleted%> deleted item<%=totalDeleted!=1?"s":""%> and cannot be undone.
                </div>
            </div>
            <form action="purgeAll" method="post" style="margin:0"
                  onsubmit="return confirm('Permanently delete ALL <%=totalDeleted%> items? This cannot be undone.')">
                <input type="hidden" name="userId" value="<%=user.getId()%>">
                <% if (hasRoadmapContext) { %>
                <input type="hidden" name="roadmapId" value="<%=roadmapId%>">
                <% } %>
                <button class="btn btn-danger" type="submit"><i class="ti ti-trash-x"></i> Empty trash</button>
            </form>
        </div>
        <% } %>
